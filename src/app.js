...
/* ---------- words data ---------- */
let W = null;
const K_WORDS = 'cv2-words', K_KEY = 'cv2-key', K_STATE = 'cv2-state';
function storedRows() {
  ...
  try {
    ...
    const v1 = JSON.parse(localStorage.getItem('chorok-voca-words-v1') || 'null');
    if (v1 && Array.isArray(v1.words) && v1.words.length) return v1.words;
  } catch (e) {}
  return null;
}
...
const Local = {
  ...
  write(s) { try { localStorage.setItem(K_STATE, JSON.stringify(s)); return true; } catch (e) { return false; } },
};
...
document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'hidden') { pauseTimers(); flush(); if (Cloud.ready) Cloud.flush(); } });
...
const Voice = {
  ...
  stop() { if (this.cur) { try { this.cur.stop(); } catch (e) {} this.cur = null; } try { speechSynthesis.cancel(); } catch (e) {} },
  async play(id) {
    ...
    if (this.available() && Sound.ctx()) {
      try {
        ...
        return;
      } catch (err) { console.warn('[초록 보카] 발음 파일 재생 실패, 기기 음성으로 대신 읽어요', err); }
    }
    await tts(e.w, 'en-US');
  },
};
...
function stageHTML(e, n) {
  return `<div class="stage" id="stage">${n > 1 ? '<div class="deck d2"></div>' : ''}${n > 0 ? '<div class="deck d1"></div>' : ''}
    ...
    <div class="stamp s-know" id="stampK">${Q ? '알아요' : '다음'}</div><div class="stamp s-dont" id="stampD">${Q ? '몰라요' : '이전'}</div></div></div>`;
}
...
async function unlock() {
  ...
  try {
    ...
    localStorage.setItem(K_WORDS, JSON.stringify({ v: data.v || 2, rev: CFG.data.rev || '', words: data.words }));
...
document.addEventListener('keydown', ev => {
  ...
  if (screen === 'quiz' && Q) {
    ...
    if (ev.key === 's' || ev.key === 'S') Voice.play(st.id);
